...
- After non-trivial workflow or code changes, run `uv run python scripts/agent_finish.py` before claiming completion. If it fails, either fix the failure or report the remaining blocker clearly.
- Keep `WORKFLOWS.md`, `CHECKS.md`, and `KNOWN_ERRORS.md` current when commands, dependencies, outputs, or known failure modes change.
