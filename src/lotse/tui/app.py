"""Lotse TUI — Interaktive Terminal-Oberfläche (Iteration 4)."""
...
class SearchScreen(Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static(
            f"[bold #f5a623]Lotse — Suche[/bold #f5a623]  [dim]Modus: {mode_label}[/dim]",
            id="search-header",
        )
        yield Input(
    ...
    def action_toggle_mode(self) -> None:
        ...
        header.update(f"[bold #f5a623]Lotse — Suche[/bold #f5a623]  [dim]Modus: {mode_label}[/dim]")
...
class RecentScreen(Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Static(
            "[bold #f5a623]Lotse — Letzte Einträge[/bold #f5a623]  "
...
class DoctorModal(ModalScreen[None]):
    ...
    def _run_checks(self) -> None:
        ...
        from lotse.core.config import DEFAULT_CONFIG_FILE, LotseConfig
...
class AuditScreen(Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        yield Static(
            "[bold #f5a623]Lotse — Audit[/bold #f5a623]  "
