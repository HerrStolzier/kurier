...
- **Duplikate**: `Store.find_routed_duplicate()` erkennt inhaltsgleiche Dateien unter anderem Namen am SHA-Fingerabdruck (`source_signature`) vor der Klassifikation — kein zweiter LLM-Call, kein zweiter Ordnereintrag. Ergebnis: `status='duplicate'`, `route_name='__duplicate__'`, `duplicate_of=<id>`. Duplikate sind aus `stats()` ausgenommen, erscheinen aber in `recent()`.
